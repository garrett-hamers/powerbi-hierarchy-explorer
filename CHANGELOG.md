...
- Add the offline sample report as a Power BI Project
  ...
  `npm run sample-report`: a TMDL model whose data is an inline Power Query
  `#table` with no external source, and this visual embedded as a private custom
  visual so nothing resolves from AppSource at open time. Packaging fails if the
  embedded copy goes stale.
