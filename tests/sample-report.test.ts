...
describe("offline AppSource sample report", () => {
  ...
  test("holds its data inline with no external source", () => {
    ...
    expect(table).toContain("partition Hierarchy = m");
    expect(table).toContain("mode: import");
    expect(table).toContain("Source = #table(");
    ...
    const model = [
      table,
      fs.readFileSync(path.join(modelDirectory, "definition", "model.tmdl"), "utf8"),
      fs.readFileSync(path.join(modelDirectory, "definition", "database.tmdl"), "utf8")
    ].join("\n");
    for (const connector of connectors) {
      expect(model).not.toContain(connector);
    }
